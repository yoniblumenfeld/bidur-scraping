...
def __insert_results_to_collection(results_by_keywords):
    ...
    for keyword,results in results_by_keywords.items():
        ...
        for doc in results:
            try:
                doc['timestamp'] = timestamp #add timestamp for each document added
...
def find_results_by_keyword(keyword):
    ...
    for relevant_collection in relevant_collections_list: results_list.extend((doc for doc in relevant_collection.find()))
